...
void MeterRead()
{
  ...
  for (int mData = 0; mData < 34; mData++) {
    ...
    if ((bitRead(meterByte[mData], 5) > 0) && (bitRead (meterByte[mData], 6) > 0)) dataAlign[mData] = meterByte[mData] >> 1; //bit 5 & 6 should be 1
    if ((bitRead(meterByte[mData], 6) > 0) && (bitRead (meterByte[mData], 7) > 0)) dataAlign[mData] = meterByte[mData] >> 2; //align 11 bit bytes
